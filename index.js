  ...
  try {
    ...
    try {
      await page.waitForSelector('div.option-list-entry--message > p.message--header', {
        visible: true,
        timeout: 9000, // Short timeout to detect the element
      });
    ...
    <html>
        <head>
            ...
            <script>
                ...
                setTimeout(function() {
                    window.location.href = './';  // Redirect to the root or current directory
                }, 2000);  // 2 seconds delay
            </script>
        </head>
        <body>
            <h1>No active Puppeteer session found. Please try again.</h1>
        </body>
    </html>
`);
...
async function validateLoginUrl(possibleUrls) {
  const browser = await launchBrowser(); // Use the existing browser instance
  const page = await browser.newPage(); // Open a new page in the existing browser
